...
訳文には翻訳時の言語が記録されるため、翻訳先を後から切り替えると、以前の言語で付いた訳は自動的に「翻訳待ち」へ戻ります（status の集計と translate の対象に反映されます）。

用語集は言語ごとに分かれます。日本語は従来どおり `.yakudoc/glossary.json`、それ以外は `.yakudoc/glossary.<code>.json`（例: `glossary.de.json`）が使われます。
