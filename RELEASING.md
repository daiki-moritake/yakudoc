...
1. `main` を最新にして、公開する 4 パッケージの `version` を揃えて上げる
   ...
   ```bash
   npm version 0.2.0 -w yakudoc-core -w yakudoc-ts-plugin -w yakudoc-ai-prep -w yakudoc-mt --no-git-tag-version
   ```

2. パッケージ間の依存(`yakudoc-core: "^0.1.0"` など)が新しいバージョンと矛盾しないか確認する
...
- ワークフローはビルドとテストを通してから publish します。途中のパッケージで失敗した場合、公開済みのバージョンは削除せず(npm は再公開を許さないため)、パッチバージョンを上げて再リリースしてください。
