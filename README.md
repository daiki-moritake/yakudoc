...
lodash・zod・`@types/node`——エディタのホバーに出てくる英語の JSDoc を、コードを 1 行も書き換えずに日本語(ほか 22 言語)で表示するツールセットです。
...
対応言語: `ja` `en` `ko` `zh` `de` `fr` `es` `pt` `it` `nl` `sv` `fi` `pl` `cs` `uk` `ru` `tr` `ar` `hi` `id` `vi` `th`(内蔵モデルの NLLB-200 / mBART-50 が両対応の言語)。`en` は原文が英語以外のパッケージ(yakudoc 自身など)を英語で読むための翻訳先です。
